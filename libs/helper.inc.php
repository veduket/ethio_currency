...
function get_cbe_data(){
  ...
  if($result['state']=="SUCCESS"){
    $dom  = new DOMDocument();
    $status = @$dom->loadHTML(mb_convert_encoding($result['data'], 'HTML-ENTITIES', "UTF-8"));
    $rate_date = strtotime($dom->getElementById("dnn_dnnCURRENTDATE_lblDate")->nodeValue);
    $patterns=array();
    $replacements=array();
    $patterns[0]="/<span(.*?)>/s";
    $patterns[1]="/<\/span>/s";
    ...
    $replacements[1]="";
    $replacements[2]="";
    $replacements[3]="assets/flags";
    $raw_html = preg_replace('/\s+/s',' ', preg_replace($patterns, $replacements, $dom->saveHTML()));
    ...
    // print_r($currency_table);exit;
    if(is_string($currency_table) && strlen($currency_table) > 0){
      $state="SUCCESS";
      $html['state']=$state;
